...
namespace WebdeskEA.Core.Configuration
{
    public class PermissionDefinitions
    {
        ...
        public PermissionDefinitions(IUserRightsRepository permissionRepo)
        {
        ...
        {
            var permissions = await _permissionRepo.GetExistingPermissinDyanmicListAsync();
        ...
        public async Task<int> PackPermissionsAsync(UserRightDto right)
        {
            var permissions = await GetPermissionsAsync();
            int packed = 0;
            ...
            foreach (var permission in permissions)
            {
                var propertyInfo = right.GetType().GetProperty($"Right{permission}");
                if (propertyInfo != null)
                {
                    ...
                    packed |= hasPermission ? 1 << await MapPermissionToIndexAsync(permission) : 0;
                }
                else
                {
